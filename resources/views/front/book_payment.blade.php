      ...
      <form method="POST" enctype="multipart/form-data" action="{{ route('front.hotel.book.payment.store', $hotel_booking) }}" id="Details" class="flex flex-col gap-6 group result-card-container">
        ...
        <div id="Contact-details" class="bg-white rounded-xl overflow-hidden flex flex-col mx-[18px]">
          <div class="flex items-center gap-4 p-4">
            <button type="button" class="flex items-center w-full gap-2 contact-name accordion-button" data-accordion="accordion-1">
              ...
              <div class="flex arrow shrink-0">
                <img src="{{ asset('assets/images/icons/arrow-up.svg')}}" class="transition-all duration-300 rotate-180" alt="arrow up">
              </div>
            </button>
          </div>
          <div id="accordion-1" class="accordion-content open">
            ...
            <div class="relative overflow-hidden">
              <img src="{{ asset('assets/images/backgrounds/note-blue-background.svg')}}" class="absolute object-cover object-top w-full h-full" alt="backgrounds">
        ...
        <div id="Transfer-details" class="bg-white rounded-xl flex flex-col mx-[18px] p-4 gap-4">
          ...
          <div class="flex flex-col gap-2 input-container">
            ...
            <div class="flex items-center gap-[10px] p-[12px_16px] rounded-lg bg-[#F8F8F8]">
              <p class="flex flex-1 font-semibold copy">Rp {{ number_format($hotel_booking->total_amount, 0, ',', '.') }}</p>
              <button type="button" class="copy-btn font-semibold text-sm leading-[21px] text-[#4041DA]">Copy</button>
            </div>
          </div>
        </div>
